...
public class TestRobot {
    ...
    public TestRobot(Electronics e, RoboLog rLog, Action act, Joystick joystick, Dashboard d, RobotState robotState, OI oi) {
        this.e = e;
        this.rLog = rLog;
        this.act = act;
    ...
    public void testPeriodic() {
        if (d.getTestMode().equals("Quick Test"))
            quickTest();
        else if (d.getTestMode().equals("Swerve Test"))
            testSwerve();
        else if (d.getTestMode().equals("Test Angle"))
            testAngle();
        else if (d.getTestMode().equals("Test Distance Sensor"))
            testDistanceSensor();
        else if (d.getTestMode().equals("Test Motors"))
            testMotors();
        else if (d.getTestMode().equals("Yellow Button"))
            rLog.print("Yellow: " + e.isPhyscalResetGyroButtonPressed());
        else if (d.getTestMode().equals("Tune Motor"))
            testTuneMotor();
    }
    ...
    public void testSwerve() {
        ...
        if (joystick1.getRawButton(1)) {
            ...
            e.zeroAllAbsoluteEncoderValues();
            rLog.print("0");
    ...
    public void testMotors() {
        //double pow = -1 * joystick1.getY();
        //put non-driving motors here
    }

    public void testTuneMotor() {
        double joystickValue = -joystick1.getY();
        int joystickPOV = joystick1.getPOV(0);
